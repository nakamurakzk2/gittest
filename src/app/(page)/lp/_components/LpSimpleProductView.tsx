...
export default function LpSimpleProductView({ productGroup, productItems, productCategories, town, business, amount, setAmount, selectedProductId, setSelectedProductId }: Props) {
  ...
  const currentProduct = productItems.find(item => item.productId === selectedProductId)
  ...
  }, [currentProduct?.purchaseConfirmations, selectedProductId])
  ...
  const reload = async () => {
    ...
    await onFetch(async () => {
      const { reachedUserLimit, count } = await UserProductPageServerLogic.fetchOwnCount(selectedProductId)
  ...
  useEffect(() => {
    reload()
  }, [selectedProductId, simpleUser])

  const onClickPurchase = () => {
    const purchaseUrl = `/product/payment?productId=${selectedProductId}&amount=${amount}`
    if (simpleUser) {
  ...
  const onClickPreConsultation = () => {
    const chatUrl = `/product/${productGroup.productGroupId}/chat?productId=${selectedProductId}`
    if (simpleUser) {
      router.push(chatUrl)
    } else {
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                {productItems.map((item, index) => (
                  <BlueButton
                    key={item.productId}
                    variant={selectedProductId === item.productId ? "default" : "destructive"}
                    onClick={() => {
